let { retrieveProduct, cartFunction } = require('./retrieveProduct');

describe("retrieveProduct", () => {
    test("SHOULD successfully retrieve a product RESPOND with status code and data", async () => {

        const expectedStatus = 200;
        const expectedData = 1;

        let req = { params: { productId: 3 } };
        ...
        cartFunction.retrieveProduct = jest.fn().mockReturnValue({ data: expectedData, status: expectedStatus });

        await retrieveProduct(req, res);
    });
    ...
    test("SHOULD log 500 error BECAUSE request was never sent to external server SENDS status code 500, and error msg and data as a json object", async function () {
        ...
        cartFunction.retrieveProduct = jest.fn().mockReturnValue(expectedResponse);
